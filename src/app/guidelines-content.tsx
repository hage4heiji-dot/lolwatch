...
// モデレーター向け判断基準の本体。モデレーター画面(要ログイン)と
// 一般公開ページ(/guidelines)の両方から参照する共通コンポーネント。
      ...
      <section className="section">
        ...
        <div className="card">
          <ul style={{ paddingLeft: "1.2rem" }}>
            <li>判定は必ず対象試合のリプレイ、および通報者が参考URL(動画・スクリーンショット等)を添付している場合はその内容を実際に確認した上で行う。一般ユーザーの通報内容(カテゴリ・コメント)はあくまで参考情報であり、それ自体を根拠に判定しない。</li>
            <li>リプレイにはチャットログが記録されないため、「暴言」「差別的発言」などチャット内容が争点になるカテゴリは、参考URLが添付されていない場合は原則「証拠不十分」とする。</li>
            <li>判定は「その通報(=その試合)」に対して行う。プレイヤー全体の印象や過去の評価で判断しない。</li>
            <li>疑わしいが確証が持てない場合は「証拠不十分」を積極的に使う。特に「チート使用疑惑」「アカウント売買・譲渡疑惑」は誤判定のコストが大きいため慎重に。</li>
            <li>「意図的な敗北行為」等、パターンの繰り返しが要件になっているカテゴリでは、1回のミス・不運だけで「違反を確認」にしない。</li>
          </ul>
        </div>
      </section>
